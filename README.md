...
「明鉴」是一款功能强大的专业暗链扫描工具，专注于检测网站、HTML文件或目录中的隐蔽链接、隐藏元素和恶意代码。该工具能够智能识别扫描目标类型（本地文件/目录、内网URL、公网URL），并自动调整扫描策略以获得最佳效果，是安全人员进行网站安全审计和应急响应的理想工具。
...
```bash
# 扫描单个HTML文件（生成文本报告）
python mingjian.py /path/to/file.html -f txt

# 扫描本地文件并启用详细日志
python mingjian.py /path/to/file.html --verbose

# 对本地文件使用高级扫描模式
python mingjian.py /path/to/file.html -m advanced -f html
```

#### 扫描本地目录
```bash
# 扫描目录（使用默认深度3）
python mingjian.py /path/to/website

# 扫描目录（设置深度为2，仅扫描当前目录和一级子目录）
python mingjian.py /path/to/website -d 2

# 扫描目录但排除特定文件或目录
python mingjian.py /path/to/website --exclude "*.log" "temp/*" "node_modules/"

# 对目录进行深度扫描并生成HTML报告
python mingjian.py /path/to/website -d 4 -f html -o detailed_reports
```

#### 扫描内网路径
```bash
# 扫描内网IP地址
python mingjian.py http://192.168.1.100

# 扫描内网域名或localhost
python mingjian.py http://localhost:8080
python mingjian.py http://internal-server.company.local

# 对内网路径进行深度扫描
python mingjian.py http://10.0.0.5 -d 3 --verbose
```

#### 扫描公网链接
```bash
# 扫描公网网站（设置爬取深度为2）
python mingjian.py https://example.com -d 2

# 对公网链接使用高级扫描模式
python mingjian.py https://example.com -m advanced -f html

# 使用代理服务器扫描公网链接
python mingjian.py https://example.com --proxy http://username:password@proxy.example.com:8080

# 公网链接扫描优化（增加线程数，使用更短的超时时间）
python mingjian.py https://example.com --threads 15 --timeout 20 --verbose
```

#### 高级使用示例
```bash
# 高级模式+自定义关键字文件+多线程+自定义超时
python mingjian.py https://example.com -m advanced --keyword-file custom_keywords.txt --threads 10 --timeout 30 --verbose

# 批量文件扫描（通过文件列表）
python mingjian.py file_list.txt -f json -o scan_results

# 轻量级快速扫描
python mingjian.py https://example.com -m basic -d 1 -t 5

# 完整扫描公网网站并生成HTML报告
python mingjian.py https://example.com -m all -d 1 -t 8 --timeout 30 -f html --verbose

# 扫描特定URL并在可疑链接详情中显示问题信息
python mingjian.py https://example.com/news.php -m all -d 1 -t 8 --timeout 30 -f html --verbose
...
python mingjian.py https://example.com -d 3 -m all -f html -o security_reports --verbose

# 针对内网网站进行安全审计
python mingjian.py http://intranet.company.local -d 4 -m all -f html -o intranet_audit
```

#### 场景2：应急响应处理
```bash
# 对疑似被黑的网站目录进行快速检查
python mingjian.py /path/to/webroot -m basic --keyword-file malware_keywords.txt --verbose

# 对被植入暗链的页面进行深度分析，使用优化的HTML报告查看可疑链接详情
python mingjian.py compromised_page.html -m advanced -f html -o incident_response
```

#### 场景3：定期安全检查
```bash
# 作为定期任务，对关键页面进行标准扫描，生成JSON报告便于自动化处理
python mingjian.py /path/to/critical_pages.txt -f json -o scan_results --verbose

# 对本地备份网站进行完整性检查
python mingjian.py /path/to/backup_site -d 3 -f json -o backup_audit
```

#### 场景4：针对特定类型链接的检测
```bash
# 自定义关键字文件，专注于检测特定类型的链接
python mingjian.py https://example.com --keyword-file custom_keywords.txt -m advanced

# 检测特定文件中的博彩网站暗链，使用HTML报告直观查看结果
python mingjian.py suspicious.html --keyword-file gambling_keywords.txt -f html
```

#### 场景5：自动化集成
```bash
# 生成JSON格式报告用于自动化处理
python mingjian.py https://example.com -f json -o automated_scans

# 通过批处理脚本批量扫描多个目标
for target in "http://site1.com" "http://site2.com" "http://192.168.1.100"; do
  python mingjian.py "$target" -d 2 -f json -o batch_results_$(date +%Y%m%d)
done
```
...
python mingjian.py https://example.com -f html -o scan_before_fix
# 进行修复操作后
python mingjian.py https://example.com -f html -o scan_after_fix
...
A: 请使用`python mingjian.py --help`查看最准确的参数信息。我们会定期更新README文档以匹配最新的命令行参数定义。
...
**Q: 扫描新闻页面时如何获得最佳结果？**
A: 对于新闻页面，建议使用以下命令：`python mingjian.py https://example.com/news.php -m all -d 1 -t 8 --timeout 30 -f html --verbose`，这样可以获得完整扫描结果并通过优化的HTML报告查看可疑链接详情。
